...
class GeoDGCNN_flow2(nn.Module):
    def __init__(self, k, emb_dims, dropout):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(96, 96, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(self.fmid, self.emb_dims, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv6 = nn.Sequential(nn.Conv1d(self.fmid + self.emb_dims, 512, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        ...
        self.feat_conv1 = GeoSetConv(2, 32)
    ...
    def forward(self, x):
        ...
        g1 = self.feat_conv1(x, geo_graph)
        ...
        #mid = torch.cat((g1, g2, g3), dim=1)  # SGNN
        #mid = torch.cat((x2, x3), dim=1)  # DGNN
        mid = torch.cat((g1, x2, x3, g2, g3), dim=1)  # SDGNN
